...
log_debug(logger,"Entre a insertar mensaje en cache!!");
	if (debugCache)
	...
	log_debug(logger,"Estoy al final de insertarMensajeEnCache");
return;
}
...
//		nodo->fin += partAux->largo;
		...
		if (partFirst == partAux)
			partFirst = nodo;
		if (debugCache)
			log_error(logger, "conshaciarriba inicio[%d] fin[%d] largo[%d]",
...
//	char * p_igual = &igual;
	...
	while (partAux) {
		...
		if (debugCache) {
			log_info(logger, "part:%d estado:%d ", particion, partAux->estado);
			mostrarCache(partFirst, ASCEND);
			dumpCache();
		}
...
//				memcpy(cache + partAux->inicio - correr, cache + partAux->inicio, partAux->largo);
				...
				partAux->fin -= correr;
				if (debugCache) {
					log_info(logger, "Corrida part:%d estado:%d ", particion,
							partAux->estado);
					mostrarCache(partFirst, ASCEND);
				}
			}
			...
			removidas++;
			if (debugCache) {
				log_info(logger, "Eliminada");/*mostrarCache(partFirst, ASCEND);*/
			}
